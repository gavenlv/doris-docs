# Doris GCP 存算分离 + FE 高可用集群部署 (公网版本)

## 概述

这是一个企业级的 **GCP 存算分离 Doris 集群** 部署方案（公网版本），实现了：
...
**本版本使用公网访问模式：**
- VM 可以访问互联网
- 安装包直接从 Apache/GitHub 下载
- 无需预上传安装包到 GCS
- 部署更简单快速

**如需内网版本（无公网访问），请使用 `doris-gcp-cluster-private` 目录。**
